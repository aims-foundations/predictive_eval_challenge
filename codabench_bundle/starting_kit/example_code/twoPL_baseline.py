#!/usr/bin/env python3
"""Two-Parameter Logistic (2PL) IRT Baseline for the Predictive AI Evaluation Challenge.

This script fits a 2PL IRT model to the training response matrix and produces
predictions for both Track 1 (response prediction) and Track 2 (robust scoring).

Usage:
    python twoPL_baseline.py --data-dir /path/to/data --output-dir ./submission

The 2PL model extends the Rasch model with item discrimination:
    P(correct | model_i, item_j) = sigmoid(a_j * (theta_i - b_j))

where:
    theta_i = ability of model i
    b_j     = difficulty of item j
    a_j     = discrimination of item j (how well the item separates abilities)
"""
...
def main():
    parser = argparse.ArgumentParser(description="2PL IRT Baseline")
    ...
    parser.add_argument("--max-epochs", type=int, default=1500, help="Maximum training epochs")
    ...
    if train_path.exists():
        ...
        # For Track 2, use the contaminated responses
        contam_path = data_dir / "contaminated_responses.csv"
        if contam_path.exists():
            rm = load_csv(str(contam_path))
            print("Using contaminated_responses.csv (Track 2 scenario)")
        else:
    ...
    # ── Fit 2PL model ────────────────────────────────────────────────────
    print(f"Fitting 2PL model ({n_subjects} subjects, {n_items} items)...")
    model = TwoPL(n_subjects=n_subjects, n_items=n_items, device=args.device)
    history = model.fit(
    ...
    # Print model summary
    with torch.no_grad():
        disc = model.discrimination.cpu().numpy()
        diff = model.difficulty.cpu().numpy()
        print(f"Discrimination: mean={disc.mean():.3f}, std={disc.std():.3f}, "
              f"range=[{disc.min():.3f}, {disc.max():.3f}]")
        print(f"Difficulty: mean={diff.mean():.3f}, std={diff.std():.3f}, "
